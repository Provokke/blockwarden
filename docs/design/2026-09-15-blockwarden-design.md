...
- **Known limitation.** The signer's 9 second budget holds for 1 or 2 URLs. The 1 second floor binds at 3, where four calls against three hung URLs take 12 seconds, and a `dependsOn` transaction's extra `eth_estimateGas` takes it to 15, past the batch's 12 second margin: the message is handed back and SQS delivers it again. Holding the budget there would need a 600 ms timeout, which healthy providers miss, so the floor stays where it is.
