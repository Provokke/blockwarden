...
5. The sweeper queries GSI2 `TXPENDING#<chainId>` every minute:
   ...
   - No receipt after the chain's stuck threshold (default 90 seconds since the last signature), and this is the next nonce to be mined: re-sign the same nonce with each fee field raised to the greater of 12.5% over the newest attempt the node did not refuse, the current estimate and geth's replacement minimum (both fields at least 10% higher and strictly higher), capped at the policy fee cap. If a refused replacement already carried fees at least that high, the raise starts from it instead. A later nonce is not replaced while a lower one is outstanding.
   - Every attempt was refused, so none is in a mempool: sign a fresh attempt at the estimate clamped to the cap, with no replacement rule, but only once the estimate's `maxFeePerGas` fits under the cap. Until then nothing is signed, so the attempt list does not grow. Nor is it signed when a refused attempt already carried fees at least as high in both fields, unless that refusal was at the cap and the new fees are under it, which is how a transaction refused during a spike recovers. Instead the highest such attempt that still has its raw bytes is rebroadcast, since the base fee may have fallen below it; one the node takes counts as live again. If none has its bytes, one attempt is signed at the highest refused fees. A fresh attempt refused outright fails the transaction and queues a filler. An accepted replacement or fresh attempt clears `feeCapReached`.
   ...
   - A rebroadcast refused outright fails the transaction and queues a filler, as the signer does; insufficient funds pauses the signer; underpriced marks it for replacement on the next sweep.
