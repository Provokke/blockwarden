...
- The signer's timeout is derived from its own Lambda timeout rather than a fixed budget. Of its 60 seconds, 4 are kept for DynamoDB and KMS and the rest splits in two: 28 seconds of RPC for one message, and a 32 second batch margin, which has to hold a whole message because a message may start with only the margin left. The worst message makes 7 calls (a `dependsOn` estimate, the cold nonce read, three for the fee estimate, the send, and the receipt check a `nonce too low` answer costs), so each call gets `clamp(28000 / (7 × urls), 1000, 2500)`: 2,500 / 2,000 / 1,333 ms for 1 / 2 / 3 URLs. At 3 URLs all hung that is 27,993 ms, inside the 28 seconds, and the 1 second floor no longer binds. viem has no minimum of its own: it arms the timer for any positive timeout. The queue's visibility timeout is six times the function timeout, 360 seconds.
