...
5. The sweeper queries GSI2 `TXPENDING#<chainId>` every minute:
   ...
   - Every attempt was refused, so none is in a mempool: sign a fresh attempt at the estimate clamped to the cap, with no replacement rule, but only once the estimate's `maxFeePerGas` fits under the cap. Until then nothing is signed, so the attempt list does not grow. A fresh attempt refused outright fails the transaction and queues a filler. An accepted replacement or fresh attempt clears `feeCapReached`.
